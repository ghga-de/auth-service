...
```bash
docker pull ghga/auth-service:9.1.0
```

Or you can build the container yourself from the [`./Dockerfile`](./Dockerfile):
```bash
# Execute in the repo's root dir:
docker build -t ghga/auth-service:9.1.0 .
```
...
docker run -p 8080:8080 ghga/auth-service:9.1.0 --help
```
...
- <a id="properties/max_iva_verification_days"></a>**`max_iva_verification_days`** *(integer)*: Maximum number of days an IVA verification code is valid. Default: `7`.
- <a id="properties/max_iva_codes_per_day"></a>**`max_iva_codes_per_day`** *(integer)*: Maximum number of verification code requests per IVA and day. Default: `3`.
